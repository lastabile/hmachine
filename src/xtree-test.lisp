        ...
        (timer 'xtree-test
          (lambda ()
            ...
            (let ((org (make-xtree-org)))
              (dotimes (i pop-size)
                (let ((gene (aref pop i)))
                  (timer 'init-g (lambda () (! (org init-g))))
                  (timer 'mutate (lambda () (! (gene mutate))))
                  (timer 'distribute (lambda () (! (gene distribute) (! (org get-g)))))
                  (timer 'run (lambda () (! (org run))))
                  (let ((k (! (org get-stored-kpis))))
                    (when (= (first k) 1.0)
                      (print (! (org get-stored-kpis))))))))))))))
