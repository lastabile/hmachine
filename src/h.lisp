...
;; Note shows the ability of hoss to handle outer defr's for establishing an evironment

(defc std-vars nil (level)
  (defr
	(defl cat (var-base level)
	  (if (= level 0) var-base (intern (format nil "~a-~a" (symbol-name var-base) level))))
	(let ((std-vars '(?this-obj ?this-rule ?this-rule-name ?root-var)))
	  (defm var-base-to-var (x)
		(if (memq x std-vars)
			(cat x level)
			x))
	  (defm base-vars ()
...
(defc objgraph graph nil
  (let ((obj-queue (make-queue))
	...
	(let ((hget-key-rest2 (rest hget-key-rest1))
	  ...
	  (defm all-matches-on-edges (rule-node obj-edges)
		(let ((rule-edges (! ((get-rule-components rule-node) preds))))
		  (let ((envlist (match-pat-obj-edge-lists rule-edges obj-edges nil nil rule-node)))	;; !!!!!!! Need to dedup this? 
			(let ((std-vars (make-std-vars (hget rule-node 'std-var-level))))
			  (let ((std-bindings `((,(! (std-vars var-base-to-var) '?this-rule) ,rule-node)
									(,(! (std-vars var-base-to-var) '?this-rule-name) ,(hget rule-node 'name))
									(,(! (std-vars var-base-to-var) '?root-var) nil)	;; This is a rootless match, so this and the obj are nil
									(,(! (std-vars var-base-to-var) '?this-obj) nil))))
		...
		(let ((envslist (query1 clauses not)))
		  (if (and vardesc (symbolp vardesc))
			  ...
			  (let ((envs (dedup-list (mapcan (lambda (envs)
												(mapcar (lambda (env)
														  (env-prune env (! ((make-std-vars 0) base-vars))))
														envs))
											  envslist))))
				(if vardesc
	  ...
	  (defm old-get-rule-neighborhood (nodes)
		(timer 'get-rule-neighborhood
		  (lambda ()
			(let ((elem-nodes
				   (dedup-list (mapcar (lambda (edge) (first edge))
			...
			(defr
			  ...
			  (let ((edges add-edges))
				(let ((r nil)
					...
					(timer 'add-consequent-edges-per-env
					  (lambda ()
						(let ((all-node-hash (make-hash-table :test #'equal))
							  ...
							  (root-var (env-lookup '?root-var env))
							  (print-list nil)
						  ...
						  (when (not (= (hash-table-count all-node-hash) 0))
							...
							(dolist (p print-list)
							  (print p))
							(setq matched-edges (cons (matched-edges pred-edges env) matched-edges))
				  ...
				  (funcall cont r (! (new-edges as-list)) matched-edges)))))))
	  ...
	  (defm update-tested (rule-node)
		...
		(let ((all-rules-tested-count (or (hget 'all-rules 'tested) 0)))
		  (rem-edge `(all-rules tested ,all-rules-tested-count))
		  (add-edge `(all-rules tested ,(+ all-rules-tested-count 1))))
		nil)
	  ...
	  (defm update-new-edges (rule-node)
		...
		(let ((new-edges-count (or (hget rule-node 'new-edges) 0)))
		  ...
		  (let ((all-rules-new-edges-count (or (hget 'all-rules 'new-edges) 0)))
			(rem-edge `(all-rules new-edges ,all-rules-new-edges-count))
			(add-edge `(all-rules new-edges ,(+ all-rules-new-edges-count 1))))
		  nil))
						...
						(let ((rule-comps (get-rule-components rule-node)))
						  (let ((pred-list (! (rule-comps preds)))
							...
							(add-consequent-edges obj-node pred-list pred-node-list add-list add-node-list not-list rule-node envlist :cont
							  (lambda (m x-new-edges x-matched-edges)
								(when m
								  ;; (print x-new-edges)
								  )
	  ...
	  (defm old-edge-elem-node-to-list (node)
		(block el
	  ...
	  ;; In this new form we'll use _elem and enforce use for only
	  ;; this purpose. We thus avoid slow checking of subqets for
	  ;; proper form.
	  ;;
	  ;;				!!!!!!!! NOTE !!!!!!!!!
	  ...
	  (defm old-list-to-edge-elem-node (edge &key ordered pushfcn (new-pool 0))
		(if ordered
			(let ((node (new-obj-node :new-pool new-pool))
				  (i 0))
	  ...
	  ;;
	  ;; 2/11/18: Experimeneted with various forms of this, including
			  ...
			  (let ((h (hmake-hash-table :size 7 :test (lambda (x y) (and (equal (first x) (first y))
				...
				(let ((possible-match-fcn (possible-match-fcn rule-node obj-node)))
				  (dolist (root-var root-vars)
					(when (funcall possible-match-fcn root-var)
				...
				(when (not (= (hhash-table-count h) 0))
				  (let ((std-vars (make-std-vars (hget rule-node 'std-var-level))))
					(let ((envlist nil))
					  (let ((std-bindings `((,(! (std-vars var-base-to-var) '?this-rule) ,rule-node)
											(,(! (std-vars var-base-to-var) '?this-rule-name) ,(hget rule-node 'name))
											(,(! (std-vars var-base-to-var) '?this-obj) ,obj-node))))
						(hmaphash (lambda (k v)
									(let ((root-var (first v))
										  (env (second v)))
									  (setq envlist (cons (append `(,@std-bindings
																	(,(! (std-vars var-base-to-var) '?root-var) ,root-var))
																  env)
														  envlist))))
								  h)
						envlist))))))))))
	...
	(defm define-rule (rule-expr &key (local nil) (new-pool 0) (add-to-local-rule-pool t))
	  (when (not (eq (first rule-expr) 'comment))
		...
		(let ((rule (define-obj :add-rule-link t :new-pool new-pool)) ;; add-rule-link nil
			  (edge-list nil)
			  (add-to-node nil)
			  (std-vars (make-std-vars new-pool)))
		  (defr
			...
			;; Return list (preds adds), consisting of new pred and add
			;; edges, where we expand any lists in the third position of
			;; an add edge into an elem-based form. This model does not
			;; directly create new nodes and augments augments the rule
			;; with appropriate new-node preds, creating corresponding
			;; vars to hold the new node, and new adds for the
			;; elem-=based edges uising the new node.
	...
	;; New version with type support. Not clear if/when this will be useful

	(defm new-possible-match-fcn (rule-node obj-node)
	  (timer 'possible-match-fcn
		(lambda ()
		  (defr
			(defl get-rule-preds-root (preds root-var) ;; Returns list (<preds-containing-root-var> <preds-containing-root-var-with-type-attr>)
			  (let ((r nil)
					(types nil))
				(dolist (pred preds)
				  (when (member root-var pred :test #'equal)
					(setq r (cons pred r))
					(when (and (= (length pred) 3)
							   (eq (second pred) 'type)
							   (not (is-var-name (third pred))))
					  (setq types (cons pred types)))))
				(list r types)))
			(defl get-obj-edges-types (obj-edges)
			  (let ((types nil))
				(dolist (obj-edge obj-edges)
				  (when (and (= (length obj-edge) 3)
							 (eq (second obj-edge) 'type))
					(setq types (cons obj-edge types))))
				types))
			;; Types are considered conjunctive, thus obj-edges must have all
			;; the types mentioned in the type-preds, but can have more as
			;; well.
			(defl check-obj-edges-types (obj-edges-types type-preds)
			  (block b1
				(dolist (type-pred type-preds)
				  (block b2
					(dolist (obj-edge-type obj-edges-types)
					  (when (eq (third type-pred) (third obj-edge-type))
						(return-from b2 nil)))
					(return-from b1 nil)))
				(return-from b1 t)))
			(let ((obj-edges (get-edges obj-node)))
			  (let ((obj-edges-types (get-obj-edges-types obj-edges)))
				(let ((obj-nodes (nodes obj-edges)))
				  (let ((rule-comps (get-rule-components rule-node)))
					(let ((preds (! (rule-comps preds))))
					  (let ((pred-const-nodes (get-rule-consts-pred preds)))
						(let ((ipo (intersect pred-const-nodes obj-nodes)))
						  (lambda (root-var)
							(bool-timer 'possible-match-true 'possible-match-false 
										(lambda ()
										  (and ipo
											   (let ((root-pred-info (get-rule-preds-root preds root-var)))
												 (let ((root-preds (first root-pred-info))
													   (root-pred-types (second root-pred-info)))
												   (when (check-obj-edges-types obj-edges-types root-pred-types)
													 (let ((root-pred-const-nodes (get-rule-consts-pred root-preds)))
													   (when am-traced
														 (print (list 'pmf root-var preds root-preds root-pred-const-nodes obj-nodes)))
													   (= 
	...
	;; Self-contained possible-match function for experimental purposes

	(defm possible-match (rule-node obj-node)
	  (block b
		(let ((f (possible-match-fcn rule-node obj-node)))
		  (let ((root-vars (get-root-vars rule-node)))
			(dolist (root-var root-vars)
			  (when (funcall f root-var)
				(return-from b t)))))))
	...
	(defm new-rule-dep-graph (&key rules except-rules)
	  (timer 'rule-dep-graph 
		(lambda ()
		  (let ((rulesq (mapcad (lambda (x)								;; Eqv to (query '((?r type rule)(?r name ?n)) '(?r ?n)), but does not create edges
								  (let ((n (hget x 'name)))
								  ...
								  (let ((r1aes (format nil "~a" r1ae)))
									(let ((r2pes (format nil "~a" r2pe)))
									  (when (or (match-one-edge r1ae r2pe nil nil nil)
										...
										(let ((f (new-obj-node)))
										  (add-edge (list r1 'xadd r1a))
										  (add-edge (list r1a 'rule-dep f))
										  (add-edge (list f 'fadd r2p))
										  (add-edge (list f 'color 'lightgreen))
										  (add-edge (list r2p 'xpred r2))
										  (add-edge (list r2p 'color 'lightblue))
										  (add-edge (list r1a 'as-string r1aes))
										  (add-edge (list r2p 'as-string r2pes)))
	...
	(defm dimensions (&optional data-fcn)
	  (let ((r  (sort (let ((e (! (edge-to-rule as-list))))
						(let ((rule-to-max (make-hash-table)))
						  (dolist (x e)
							(let ((h (make-hash-table)))
							  (dolist (y (second x))
								(setf (gethash (third y) h) (+ (or (gethash (third y) h) 0) 1)))
							  (maphash (lambda (k v)
										 (let ((rule-name k)
											   (edge-max v))
				...
				(let ((node (node-info-node ni))
				  ...
				  (let ((rule-node node))
					(mapcar (lambda (node) (make-node-info :node node :type :rule))
							(mapunion (lambda (x) (hget-inverse-all x 'pred))
									  (mapunion (lambda (x) (hget-all x 'rule-dep))
																					 ...
																					 (let ((envno 0))
																					   (let ((rule-name (hget rule-node 'name)))
																						 (dolist (matched-edges matched-edges-list)
																						   (dolist (matched-edge matched-edges)
																							 (! (edge-to-rule insert) matched-edge (list seqno envno rule-name)))
																						   (setq envno (+ envno 1))))))))
																			   (when (eq m :new-edges)
																				 (let ()
																				   (dolist (edge-info e)
...
;; Would be more accurate to compare based on env-equal. Instead it's
;; order-based, which seems to work out but is not ideal.

(defc env-triggered nil nil
  (let ((env-triggered-table (make-hash-table :test #'equalp :size 1021))
	...
	(defm env-remove-bindings (env)
	  (let ((r nil))
		(dolist (binding env)
		  (when (not (or (eq (first binding) '?root-var)
						 (eq (first binding) '?this-obj)))
			(setq r (append r (list binding)))))
		r))
...
(defc dumper nil ()
  (let ((g nil)
	...
	(defm dump-gv-edges (file &key
							  ...
							  (emit-legend t)
							  (single-graph nil)						;; If true, generates a subgraph per rule, but not s separate digraph perf rule.
...
(let ((h (make-hash-table :test #'equal)))
  (defun cross-aux2 (envs-list &key (record-lengths nil))
	(timer 'cross-aux2
	  (lambda ()
		(let ((lengths (and record-lengths
							(list (length envs-list) (mapcar (lambda (x) (length x)) envs-list)))))
...
(defun old-old-cross-aux2 (envs-list &key record-lengths)
  (timer 'cross-aux2
	(lambda ()
	  (let ((lengths (and record-lengths
						  (list (length envs-list) (mapcar (lambda (x) (length x)) envs-list)))))
...
;; This one looks fastest, about 1.7 usec as opposed to the two below, old and old-old, at 4 usec and 12 usec respectively. Sometimes simplest is best!
...
(let ((bindinghash (make-hash-table :size 127 :test #'equal)))
  (defun old-env-no-conflict-dedup (env)
	(timer 'env-no-conflict-dedup
	  (lambda ()
		(block b
		  (clrhash bindinghash)
...
(let ((bindinghash (hmake-hash-table :size 127 :test #'equal)))
  (defun old-old-env-no-conflict-dedup (env)
	(timer 'env-no-conflict-dedup
	  (lambda ()
		(block b
		  (hclrhash bindinghash)
		  (dolist (binding env)
			(let ((var (first binding))
				  (val (second binding)))
			  (let ((hval (hgethash var bindinghash)))
				(when (null hval)
				  (setf (hgethash var bindinghash) val)
				  (setq hval val))
				(when (not (equal val hval))
				  (return-from b nil)))))
		  (let ((r nil))
	  ...
	  (defg
		'(
		  ...
		  (rule
		   ...
		   (add
			(print std-notes)
			(note footer "Copyright (c) 2018 Lawrence Stabile"))
		   (del
			(global-node rule ?this-rule)))
